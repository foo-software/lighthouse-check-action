...
var _default = async ({
  ...
  try {
    let markdown = ''; // we'll create a way to uniquely identify a comment so that we don't edit
    // the wrong one

    const commentIds = [];
    results.forEach(result => {
      commentIds.push(result.id || `${result.emulatedFormFactor}:${result.url}`); // badges
    ...
    if (isLocalAudit) {
      ...
      if (isGitHubAction) {
        markdown += '(https://www.foo.software/docs/lighthouse-check-github-action/examples#running-on-foo-and-saving-results)\n';
      } else if (isOrb) {
    ...
    const commentIdentifier = `\n<!-- COMMENT_ID${JSON.stringify(commentIds)}COMMENT_ID -->`;
    markdown += commentIdentifierPrefix + commentIdentifier; // establish existing comment
    ...
    if (!prCommentSaveOld) {
      ...
      if (Array.isArray(existingCommentsJsonResult) && existingCommentsJsonResult.length) {
        existingComment = existingCommentsJsonResult.find(current => {
          const hasLighthouseComment = current.body.includes(commentIdentifierPrefix);
          ...
          const [, commentIdsFromExistingCommentString] = current.body.split('COMMENT_ID');

          if (!commentIdsFromExistingCommentString) {
            return false;
          }

          const commentIdsFromExistingComment = JSON.parse(commentIdsFromExistingCommentString); // if one has more results than the other then we are definitely different
